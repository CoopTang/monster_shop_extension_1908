...
RSpec.describe 'As a registered user' do
  describe 'When I visit a profile order show page' do
    ...
    it 'does not display a link if the order is shipped or cancelled' do
      ...
      order_1 = user.orders.create!(name: 'Meg', address_id: home.id, status: 2)
    ...
    it 'displays a link to edit the shipping address of an order only if the order is pending' do
      meg = Merchant.create(name: "Meg's Bike Shop", address: '123 Bike Rd.', city: 'Denver', state: 'CO', zip: 80_203)
      brian = Merchant.create(name: "Brian's Dog Shop", address: '125 Doggo St.', city: 'Denver', state: 'CO', zip: 80_210)
      ...
      order_1 = user.orders.create!(name: 'Meg', address_id: home.id, status: 0)
      order_2 = user.orders.create!(name: 'Meg', address_id: home.id, status: 2)
      ...
      item_order_3 = order_2.item_orders.create!(item: tire, price: tire.price, quantity: 2)
      item_order_4 = order_2.item_orders.create!(item: pull_toy, price: pull_toy.price, quantity: 3, status: 1)
